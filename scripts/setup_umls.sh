 ...
 *0 -> do not install
  ...
  3 -> download and install zipped source code version (508MB) (incomplete)
  4 -> download and install latest SVN source code version (1.3GB) (incomplete)
...
data_server="172.22.4.129"
data_server_ftp="/var/ftp/pub"
server_key="$HOME/.ssh/cades_keys/cades_server_172-22-4-129"
...
# Requires: run_cmd(), uname
# Inputs:
# Outputs: distro
# NOTE: verify CentOS check and use a more portable approach
check_distro()
{
    local distro
    if [ "$(uname -v | grep -i 'centos')" ]; then
        distro="centos"
    else
        distro="ubuntu"
    fi
...
if [ $ctakes_mode -gt 0 ]; then
    ...
    if [ $ctakes_mode -eq 1 ] || [ $ctakes_mode -eq 2 ]; then
        if [ ! -d "$ctakes_install_dir" ]; then
            ...
            # Download from CADES server
                ...
                if [ ! -f "$tmp_install_dir/$ctakes_zip" ]; then
                    run_cmd "scp -i $server_key $USER@$data_server:$ctakes_url $tmp_install_dir"
                fi
            fi
    ...
    # NOTE: incomplete, need to compile source code?
    elif [ $ctakes_mode -eq 3 ]; then
        ctakes_zip="${ctakes_dir}-src.tar.gz"
        ctakes_url="http://apache.mirrors.ionfish.org/ctakes/$ctakes_ver/$ctakes_zip"
        if [ ! -d "$ctakes_install_dir" ]; then
            if [ ! -f "$ctakes_zip" ]; then
                run_cmd "wget $ctakes_url"
            fi
            uncompress_file "$ctakes_zip"
            run_cmd "sudo mv $ctakes_dir $ctakes_install_dir"
            run_cmd "sudo rm $ctakes_zip"
        fi

    # Download latest SVN source code
    # NOTE: incomplete, need to compile source code?
    elif [ $ctakes_mode -eq 4 ]; then
        ctakes_url="https://svn.apache.org/repos/asf/ctakes/trunk"
        if [ ! -d "$ctakes_install_dir" ]; then
            run_cmd "svn co $ctakes_url"
            run_cmd "sudo mv trunk $ctakes_install_dir"
        fi
    fi
    ...
    if [ $ducc_mode -eq 1 ] || [ $ducc_mode -eq 2 ]; then
        if [ ! -d "$ducc_install_dir" ]; then
            ...
            # Download from CADES server
                ...
                if [ ! -f "$tmp_install_dir/$ducc_zip" ]; then
                    run_cmd "scp -i $server_key $USER@$data_server:$ducc_url $tmp_install_dir"
                fi
            fi
...
if [ $umls_mode -gt 0 ]; then
    ...
    if [ $umls_mode -eq 1 ] || [ $umls_mode -eq 2 ]; then
        ...
        # Download from CADES server
            ...
            if [ ! -f "$tmp_install_dir/$umls_zip" ]; then
                run_cmd "scp -i $server_key $USER@$data_server:$umls_url $tmp_install_dir"
            fi
        fi
        uncompress_file "$tmp_install_dir/$umls_zip" "$tmp_install_dir"
        run_cmd "sudo mv $tmp_install_dir/resources $ctakes_install_dir"
        #run_cmd "sudo cp -r $tmp_install_dir/resources $ctakes_install_dir"
